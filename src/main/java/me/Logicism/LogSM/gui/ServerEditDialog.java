...
public class ServerEditDialog extends JDialog {
    ...
    public ServerEditDialog(Server server) {
        this.server = server;
        ...
        textField1.setText(server.getName());
        comboBox1.setSelectedItem(server.getServerType().toString());
        ...
        comboBox2.setSelectedItem(server.getVersion());
    ...
    private void onOK() {
        ...
        if (textField1.getText().length() > 3) {
            ...
            if (!server.getName().toLowerCase().equals(textField1.getText().toLowerCase()) && exists) {
